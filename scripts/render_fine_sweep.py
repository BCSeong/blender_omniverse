"""Fine-only coverage sweep: roughness=0.10, medium/large fixed at base.

  Cov4x:  12K fine + 1200 med + 120 large (previous best, reference)
  Fine6x: 18K fine + 300 med + 30 large
  Fine8x: 24K fine + 300 med + 30 large
  Fine10x:30K fine + 300 med + 30 large
"""
import asyncio
import json
import os
...
SWEEP_VARIANTS = [
    {"name": "Cov4x",   "texture": "sphere_M1_dust_weight_4x.png",  "desc": "4x all (12K+1200+120, reference)"},
    {"name": "Fine6x",  "texture": "sphere_M1_dust_fine_6x.png",    "desc": "6x fine only (18K+300+30)"},
    {"name": "Fine8x",  "texture": "sphere_M1_dust_fine_8x.png",    "desc": "8x fine only (24K+300+30)"},
    {"name": "Fine10x", "texture": "sphere_M1_dust_fine_10x.png",   "desc": "10x fine only (30K+300+30)"},
]
...
def _log(msg):
    print(f"[fine-sweep] {msg}", flush=True)
...
async def main():
    ...
    try:
        ...
        summary_path = os.path.join(BASE_OUTPUT, "fine_sweep_summary.json")

"""M1 (DustyMirror.mdl) parameter sweep in Kit.

Sweep matrix (new particle-based dust_weight texture):
  M1a: roughness=0.3, influence=1.0  (narrow scatter, texture-only)
  M1b: roughness=0.4, influence=1.0  (moderate scatter, texture-only)
  M1c: roughness=0.5, influence=1.0  (wide scatter, texture-only)
  M1d: roughness=0.4, constant=0.08, influence=0.5  (base+texture blend)

Renders 4_mid_N variant for each. Output: M1{x}_sweep/4_mid_N.png
"""
import asyncio
import json
import os
...
SWEEP_VARIANTS = [
    {
        ...
        "dust_weight_texture_influence": 1.0,
        "desc": "narrow scatter, texture-only",
    },
    {
        "name": "M1b",
        "dust_roughness": 0.4,
        "dust_weight_constant": 0.04,
        "dust_weight_texture_influence": 1.0,
        "desc": "moderate scatter, texture-only",
    },
    {
        "name": "M1c",
        "dust_roughness": 0.5,
        "dust_weight_constant": 0.04,
        "dust_weight_texture_influence": 1.0,
        "desc": "wide scatter, texture-only",
    },
    {
        "name": "M1d",
        "dust_roughness": 0.4,
        "dust_weight_constant": 0.08,
        "dust_weight_texture_influence": 0.5,
        "desc": "moderate scatter, base+texture blend",
    },
]
...
def _log(msg):
    print(f"[m1-sweep] {msg}", flush=True)
...
async def render_m1_sweep():
    ...
    try:
        ...
        # --- Set DustyMirror.mdl once (MDL compile) ---
        _log("\nSetting DustyMirror.mdl with new particle texture...")
        _clear_all_inputs(shader)
        _cleanup_sub_prims(stage)
        ...
        for v in SWEEP_VARIANTS:
            ...
            _log(f"  roughness={v['dust_roughness']}, "
                 f"constant={v['dust_weight_constant']}, "
                 f"influence={v['dust_weight_texture_influence']}")
            _log(f"{'='*50}")

            # Only change sweep parameters (no MDL recompile needed)
            shader.CreateInput(
                "dust_roughness", Sdf.ValueTypeNames.Float
            ).Set(v["dust_roughness"])
            shader.CreateInput(
                "dust_weight_constant", Sdf.ValueTypeNames.Float
            ).Set(v["dust_weight_constant"])
            shader.CreateInput(
                "dust_weight_texture_influence", Sdf.ValueTypeNames.Float
            ).Set(v["dust_weight_texture_influence"])

            # Reset accumulation and settle
            settings.set_bool("/rtx/pathtracing/resetAccumulation", True)
            _log(f"  Settling ({PARAM_SETTLE_FRAMES} frames)...")
            await _wait_frames(PARAM_SETTLE_FRAMES)
            ...
            results[name] = {
                ...
                "render_time_s": dt,
                "params": {
                    "dust_roughness": v["dust_roughness"],
                    "dust_weight_constant": v["dust_weight_constant"],
                    "dust_weight_texture_influence": v["dust_weight_texture_influence"],
                },
            }
        ...
        summary = {
            ...
            "texture": "sphere_M1_dust_weight.png (1024x1024, particle-based)",
            "methods": results,
            "settings": {
                ...
                "firefly_after": {
                    k: v for k, v in firefly_after.items() if v is not None
                },
            },
        }
        summary_path = os.path.join(BASE_OUTPUT, "m1_sweep_summary.json")
...
asyncio.ensure_future(render_m1_sweep())

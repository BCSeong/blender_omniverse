"""M1 sweep round 2: lower roughness + higher dust weight.

Based on round 1 finding: lower roughness = more visible dust.
Now testing roughness 0.10-0.25 with boosted texture.

  M1e: roughness=0.10, influence=1.0  (very narrow scatter)
  M1f: roughness=0.15, influence=1.0  (narrow scatter)
  M1g: roughness=0.20, influence=1.0  (narrow-moderate)
  M1h: roughness=0.25, influence=1.0  (moderate-narrow)
"""
import asyncio
import json
import os
...
def _log(msg):
    print(f"[m1-sweep2] {msg}", flush=True)
...
async def render_m1_sweep2():
    ...
    try:
        ...
        summary_path = os.path.join(BASE_OUTPUT, "m1_sweep2_summary.json")
...
asyncio.ensure_future(render_m1_sweep2())
